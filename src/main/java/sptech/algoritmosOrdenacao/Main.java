...
public class Main {
    ...
    public static void pesquisaBinariaIterativa(int[] vetor, int numProcurado){
        int i = 0, j = 0;while(i < vetor.length){
            if(vetor[i] == numProcurado){
                System.out.println("Número " + numProcurado + "encontrado após " + i + "iterações");
                return;
            }
            i++;
        }
        System.out.println("Numero " + numProcurado + "não encontrado mesmo após " + i + "iterações");
    }
    ...
    public static void pesquisabinariaRecursiva(int[] vetor, int numProcurado, int qtdIteracoes){
        if(vetor.length == 1 && vetor[0]==numProcurado){
            System.out.println("Numero " + numProcurado + "não encontrado mesmo após " + qtdIteracoes + "iterações");
            return;
        }
        int meio = vetor[vetor.length/2];
        
        if(meio == numProcurado){
            System.out.println("Número " + numProcurado + "encontrado após " + qtdIteracoes + "iterações");
        }
